...
public class SpecialAbilityHUD : MonoBehaviour
{
    ...
    [SerializeField] private Color solarVortexColor = new Color(1f, 0.35f, 0.04f, 0.95f);
    [SerializeField] private Color sawBarrierColor = new Color(0.9f, 0.95f, 1f, 0.95f);
    [SerializeField] private Color clusterBombsColor = new Color(1f, 0.65f, 0.08f, 0.95f);
    ...
    private AbilitySlot solarVortexSlot;
    private AbilitySlot sawBarrierSlot;
    private AbilitySlot clusterBombsSlot;
    ...
    private void BuildHUD()
    {
        ...
        solarVortexSlot = CreateAbilitySlot(rootObject.transform, "SOL", "Solar+", solarVortexColor);
        sawBarrierSlot = CreateAbilitySlot(rootObject.transform, "SAW", "Saw+", sawBarrierColor);
        clusterBombsSlot = CreateAbilitySlot(rootObject.transform, "CLU", "Cluster+", clusterBombsColor);
    }
    ...
    private void RefreshHUD()
    {
        ...
        bool hasSolarVortex = playerSpecialAbilities.SolarVortexUnlocked;
        bool hasSawBarrier = playerSpecialAbilities.SawBarrierUnlocked;
        bool hasClusterBombs = playerSpecialAbilities.ClusterBombsUnlocked;
        ...
        SetSlotVisible(solarVortexSlot, hasSolarVortex);
        SetSlotVisible(sawBarrierSlot, hasSawBarrier);
        SetSlotVisible(clusterBombsSlot, hasClusterBombs);
        ...
        {
            hasAnyAbility = true;
            solarVortexSlot.DetailText.text = $"{playerSpecialAbilities.SolarVortexDamagePerTick:0}/tick";
            solarVortexSlot.BackgroundImage.color = solarVortexColor;
        }

        if (hasSawBarrier)
        {
            hasAnyAbility = true;
            sawBarrierSlot.DetailText.text = $"x{playerSpecialAbilities.SawBarrierCount}";
            sawBarrierSlot.BackgroundImage.color = sawBarrierColor;
        }

        if (hasClusterBombs)
        {
            hasAnyAbility = true;
            clusterBombsSlot.DetailText.text = $"x{playerSpecialAbilities.ClusterBombSecondaryBlastCount}";
            clusterBombsSlot.BackgroundImage.color = clusterBombsColor;
        }
    ...
    private void SetAllSlotsVisible(bool visible)
    {
        ...
        SetSlotVisible(solarVortexSlot, visible);
        SetSlotVisible(sawBarrierSlot, visible);
        SetSlotVisible(clusterBombsSlot, visible);
    }
