...
public class RewardChestDropper : MonoBehaviour
{
    ...
    [SerializeField] private float rewardPopupDuration = 1.65f;
    ...
    private void TryDropChest()
    {
        ...
        if (chestObject == null)
        {
            Debug.LogWarning($"{gameObject.name} failed to spawn a reward chest.");
            return;
        }
        ...
        if (showRewardPopup)
        {
            SpecialRewardAvailablePopupUI.ShowGlobal(rewardTier, rewardPopupDuration);
        }
    }
    ...
    private Vector3 GetChestSpawnPosition()
    {
        ...
        return transform.position + spawnOffset + new Vector3(
            randomOffset.x,
            randomOffset.y,
            0f
        );
