...
public class SpecialUpgradeManager : MonoBehaviour
{
    ...
    private void BuildMiniBossRewardPool()
    {
        ...
        miniBossRewardPool.Add(new SpecialUpgradeOption(
            "EVOLUTION: Solar Vortex",
            "Consumes Fire Trail + Black Hole to create a burning gravity field.",
            SpecialUpgradeType.SolarVortex,
            1f
        ));

        miniBossRewardPool.Add(new SpecialUpgradeOption(
            "EVOLUTION: Saw Barrier",
            "Consumes Orbiting Blade + Guardian Shield to create a damaging defensive saw wall.",
            SpecialUpgradeType.SawBarrier,
            1f
        ));

        miniBossRewardPool.Add(new SpecialUpgradeOption(
            "EVOLUTION: Cluster Bombs",
            "Consumes Explosive Shots + Shrapnel Mines to create chained secondary blasts.",
            SpecialUpgradeType.ClusterBombs,
            1f
        ));
    ...
    private bool IsRewardOptionEligible(SpecialUpgradeOption option)
    {
        ...
        if (IsEvolutionRewardType(option.Type))
        {
            return IsEvolutionRewardEligible(option.Type);
        }
    ...
    private bool IsEvolutionRewardType(SpecialUpgradeType upgradeType)
    {
        switch (upgradeType)
        {
            case SpecialUpgradeType.CryoBlast:
            case SpecialUpgradeType.SolarVortex:
            case SpecialUpgradeType.SawBarrier:
            case SpecialUpgradeType.ClusterBombs:
                return true;
        }
    ...
    private bool IsEvolutionRewardEligible(SpecialUpgradeType upgradeType)
    {
        if (playerSpecialAbilities == null)
        {
            return false;
        ...
        {
            case SpecialUpgradeType.CryoBlast:
                return playerSpecialAbilities.CryoBlastUnlocked || playerSpecialAbilities.CanEvolveCryoBlast();
            case SpecialUpgradeType.SolarVortex:
                return playerSpecialAbilities.SolarVortexUnlocked || playerSpecialAbilities.CanEvolveSolarVortex();
            case SpecialUpgradeType.SawBarrier:
                return playerSpecialAbilities.SawBarrierUnlocked || playerSpecialAbilities.CanEvolveSawBarrier();
            case SpecialUpgradeType.ClusterBombs:
                return playerSpecialAbilities.ClusterBombsUnlocked || playerSpecialAbilities.CanEvolveClusterBombs();
        }
    ...
    private void RefreshRewardButtons()
    {
        ...
        for (int i = 0; i < rewardButtons.Length; i++)
        {
            ...
            SetButtonText(i, GetRewardButtonText(option));
    ...
    private string GetRewardButtonText(SpecialUpgradeOption option)
    {
        if (option == null)
        {
            return string.Empty;
        ...
        if (playerSpecialAbilities != null && IsEvolutionRewardType(option.Type))
        {
            string evolvedUpgradeText = GetOwnedEvolutionUpgradeText(option.Type);

            if (!string.IsNullOrEmpty(evolvedUpgradeText))
            {
                return evolvedUpgradeText;
            }
        }
    ...
    private string GetOwnedEvolutionUpgradeText(SpecialUpgradeType upgradeType)
    {
        if (playerSpecialAbilities == null)
        {
            return string.Empty;
        ...
        switch (upgradeType)
        {
            case SpecialUpgradeType.CryoBlast:
                ...
                {
                    return "CRYO BLAST\n+12 damage, +0.25 radius, +0.75 knockback, -0.20s cooldown";
                }
                break;

            case SpecialUpgradeType.SolarVortex:
                if (playerSpecialAbilities.SolarVortexUnlocked)
                {
                    return "SOLAR VORTEX\n+4 damage/tick, +0.22 radius, +5 pull, +0.18s duration, -0.18s cooldown";
                }
                break;

            case SpecialUpgradeType.SawBarrier:
                if (playerSpecialAbilities.SawBarrierUnlocked)
                {
                    if (playerSpecialAbilities.SawBarrierCount < playerSpecialAbilities.MaxSawBarrierCount)
                    {
                        return "SAW BARRIER\n+1 shield, +6 damage, +14 speed, +0.03 hit radius, faster hits";
                    }

                    return "SAW BARRIER\n+6 damage, +14 speed, +0.03 hit radius, faster hits";
                }
                break;

            case SpecialUpgradeType.ClusterBombs:
                if (playerSpecialAbilities.ClusterBombsUnlocked)
                {
                    if (playerSpecialAbilities.ClusterBombSecondaryBlastCount < 8)
                    {
                        return "CLUSTER BOMBS\n+8% blast damage, +0.12 radius, +1 secondary blast";
                    }

                    return "CLUSTER BOMBS\n+8% blast damage, +0.12 radius";
                }
                break;
        }
    ...
    private void ApplySpecialUpgrade(SpecialUpgradeOption upgrade)
    {
        ...
        switch (upgrade.Type)
        {
            ...
            case SpecialUpgradeType.SolarVortex:
                ApplySolarVortexUpgrade();
                break;

            case SpecialUpgradeType.SawBarrier:
                ApplySawBarrierUpgrade();
                break;

            case SpecialUpgradeType.ClusterBombs:
                ApplyClusterBombsUpgrade();
                break;
        }
    }
    ...
    private void ApplySolarVortexUpgrade()
    {
        if (playerSpecialAbilities == null)
        {
            Debug.LogWarning("SpecialUpgradeManager could not find PlayerSpecialAbilities.");
        ...
        if (!playerSpecialAbilities.SolarVortexUnlocked)
        {
            playerSpecialAbilities.UnlockSolarVortex();
            return;
        }

        playerSpecialAbilities.IncreaseSolarVortexDamage(4f);
        playerSpecialAbilities.IncreaseSolarVortexRadius(0.22f);
        playerSpecialAbilities.IncreaseSolarVortexPullStrength(5f);
        playerSpecialAbilities.IncreaseSolarVortexDuration(0.18f);
        playerSpecialAbilities.ReduceSolarVortexCooldown(0.18f);
    }

    private void ApplySawBarrierUpgrade()
    {
        if (playerSpecialAbilities == null)
        {
            Debug.LogWarning("SpecialUpgradeManager could not find PlayerSpecialAbilities.");
        ...
        if (!playerSpecialAbilities.SawBarrierUnlocked)
        {
            playerSpecialAbilities.UnlockSawBarrier();
            return;
        }

        if (playerSpecialAbilities.SawBarrierCount < playerSpecialAbilities.MaxSawBarrierCount)
        {
            playerSpecialAbilities.AddSawBarrierShield(1);
        }

        playerSpecialAbilities.IncreaseSawBarrierDamage(6f);
        playerSpecialAbilities.IncreaseSawBarrierOrbitSpeed(14f);
        playerSpecialAbilities.IncreaseSawBarrierHitRadius(0.03f);
        playerSpecialAbilities.ReduceSawBarrierHitCooldown(0.01f);
    }

    private void ApplyClusterBombsUpgrade()
    {
        if (playerSpecialAbilities == null)
        {
            Debug.LogWarning("SpecialUpgradeManager could not find PlayerSpecialAbilities.");
        ...
        if (!playerSpecialAbilities.ClusterBombsUnlocked)
        {
            playerSpecialAbilities.UnlockClusterBombs();
            return;
        }

        playerSpecialAbilities.IncreaseClusterBombDamage(0.08f);
        playerSpecialAbilities.IncreaseClusterBombRadius(0.12f);

        if (playerSpecialAbilities.ClusterBombSecondaryBlastCount < 8)
        {
            playerSpecialAbilities.AddClusterBombSecondaryBlast(1);
        }
    }
