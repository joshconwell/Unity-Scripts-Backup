...
public class EliteSpawnDirector : MonoBehaviour
{
    ...
    [Header("Elite Warning Toast")]
    [SerializeField] private EliteWarningToastUI eliteWarningToastUI;
    [SerializeField] private bool showEliteWarningToast = true;
    [SerializeField] private float eliteWarningToastDuration = 1.55f;
    [SerializeField] private string fallbackEliteName = "ELITE ENEMY";
    ...
    [Header("Old Warning Object Optional")]
    [SerializeField] private bool showWarningBeforeSpawn = false;
    ...
    private IEnumerator EliteSpawnRoutine(bool debugSpawn)
    {
        ...
        string eliteName = GetEliteDisplayName(elitePrefab);

        if (showEliteWarningToast)
        {
            FindEliteWarningToastIfNeeded();

            if (eliteWarningToastUI != null)
            {
                eliteWarningToastUI.ShowWarning(eliteName, eliteWarningToastDuration);
            }
        }
        ...
        float totalWarningDuration = 0f;

        if (showEliteWarningToast)
        {
            totalWarningDuration = Mathf.Max(totalWarningDuration, eliteWarningToastDuration);
        }

        if (showWarningBeforeSpawn && warningObject != null)
        {
            totalWarningDuration = Mathf.Max(totalWarningDuration, warningDuration);
        }

        if (totalWarningDuration > 0f)
        {
            yield return new WaitForSeconds(totalWarningDuration);
        }
    ...
    private string GetEliteDisplayName(GameObject elitePrefabObject)
    {
        if (elitePrefabObject == null)
        {
            return fallbackEliteName;
        }

        EliteDisplayName displayName = elitePrefabObject.GetComponent<EliteDisplayName>();

        if (displayName == null)
        {
            displayName = elitePrefabObject.GetComponentInChildren<EliteDisplayName>();
        }
        ...
        string cleanedName = elitePrefabObject.name;
    ...
    private void FindEliteWarningToastIfNeeded()
    {
        if (eliteWarningToastUI != null)
        {
            return;
        ...
        eliteWarningToastUI = FindObjectOfType<EliteWarningToastUI>();
    }
    ...
    private void OnDisable()
    {
        ...
        if (eliteWarningToastUI != null)
        {
            eliteWarningToastUI.HideInstant();
        }
    }
}