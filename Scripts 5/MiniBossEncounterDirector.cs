...
public class MiniBossEncounterDirector : MonoBehaviour
{
    ...
    [SerializeField] private EnemyPressureEventDirector pressureEventDirector;
    [SerializeField] private bool cancelPressureEventWhenMiniBossStarts = true;
    [SerializeField] private bool pausePressureEventsDuringMiniBoss = true;

    [Header("Mini Boss - Old Single Prefab Fallback")]
    [SerializeField] private GameObject miniBossPrefab;

    [Header("Mini Boss - Random Pool")]
    [SerializeField] private GameObject[] miniBossPrefabs;
    [SerializeField] private bool useRandomMiniBossPrefab = true;
    [SerializeField] private bool avoidSameBossTwiceInARow = true;

    [Header("Intro Banner")]
    [SerializeField] private MiniBossIntroBannerUI miniBossIntroBannerUI;
    [SerializeField] private bool showIntroBanner = true;
    [SerializeField] private float introBannerDuration = 2f;
    [SerializeField] private string fallbackBossName = "UNKNOWN THREAT";
    ...
    private int lastMiniBossPrefabIndex = -1;

    private GameObject selectedMiniBossPrefabForEncounter;
    private GameObject currentBossObject;
    private Health currentBossHealth;

    public bool EncounterActive => encounterActive;
    public bool BossHasSpawned => bossHasSpawned;
    ...
    private IEnumerator MiniBossEventRoutine()
    {
        ...
        selectedMiniBossPrefabForEncounter = GetSelectedMiniBossPrefab();

        if (selectedMiniBossPrefabForEncounter == null)
        {
        ...
        string selectedBossName = GetBossDisplayName(selectedMiniBossPrefabForEncounter);

        Debug.Log($"Mini boss event started. Selected boss: {selectedBossName}");

        PausePressureEventsForMiniBoss();
        ...
        if (showIntroBanner)
        {
            FindIntroBannerIfNeeded();

            if (miniBossIntroBannerUI != null)
            {
                miniBossIntroBannerUI.ShowIntro(selectedBossName, introBannerDuration);
            }
        }
        ...
        float totalPreBossDelay = Mathf.Max(
            warningDuration,
            enemyRetreatDuration + bossSpawnDelayAfterRetreat,
            introBannerDuration
        );
    ...
    private void PausePressureEventsForMiniBoss()
    {
        if (!pausePressureEventsDuringMiniBoss && !cancelPressureEventWhenMiniBossStarts)
        {
            return;
        }

        FindPressureEventDirectorIfNeeded();

        if (pressureEventDirector == null)
        {
            return;
        }

        if (pausePressureEventsDuringMiniBoss)
        {
            pressureEventDirector.PausePressureEvents(cancelPressureEventWhenMiniBossStarts);
            return;
        }

        if (cancelPressureEventWhenMiniBossStarts)
        {
            pressureEventDirector.CancelActivePressureEvent();
        }
    }

    private void ResumePressureEventsAfterMiniBoss()
    {
        if (!pausePressureEventsDuringMiniBoss)
        {
            return;
        ...
        pressureEventDirector.ResumePressureEvents();
    }

    private void SpawnMiniBoss()
    {
        GameObject selectedMiniBossPrefab = selectedMiniBossPrefabForEncounter;

        if (selectedMiniBossPrefab == null)
        {
            selectedMiniBossPrefab = GetSelectedMiniBossPrefab();
        }
    ...
    private GameObject GetSelectedMiniBossPrefab()
    {
        if (useRandomMiniBossPrefab && miniBossPrefabs != null && miniBossPrefabs.Length > 0)
        {
            List<int> validIndexes = new List<int>();

            for (int i = 0; i < miniBossPrefabs.Length; i++)
            {
                if (miniBossPrefabs[i] == null)
                {
                    continue;
                }

                if (avoidSameBossTwiceInARow && miniBossPrefabs.Length > 1 && i == lastMiniBossPrefabIndex)
                {
                    continue;
                }
            ...
            if (validIndexes.Count > 0)
            {
                ...
                int selectedPrefabIndex = validIndexes[randomListIndex];

                lastMiniBossPrefabIndex = selectedPrefabIndex;

                return miniBossPrefabs[selectedPrefabIndex];
            }
        }
    ...
    private string GetBossDisplayName(GameObject bossPrefab)
    {
        if (bossPrefab == null)
        {
        ...
        MiniBossDisplayName displayName = bossPrefab.GetComponent<MiniBossDisplayName>();

        if (displayName == null)
        {
            displayName = bossPrefab.GetComponentInChildren<MiniBossDisplayName>();
        }

        if (displayName != null && !string.IsNullOrEmpty(displayName.DisplayName))
        {
            return displayName.DisplayName.ToUpper();
        }

        string cleanedName = bossPrefab.name;
        cleanedName = cleanedName.Replace("(Clone)", "");
        cleanedName = cleanedName.Replace("_", " ");
        cleanedName = cleanedName.Replace("-", " ");
        cleanedName = cleanedName.Trim();

        if (cleanedName.StartsWith("Boss "))
        {
            cleanedName = cleanedName.Substring(5);
        }

        if (!cleanedName.StartsWith("THE "))
        {
            cleanedName = "THE " + cleanedName;
        }

        return cleanedName.ToUpper();
    }
    ...
    private IEnumerator EndMiniBossEventRoutine()
    {
        ...
        selectedMiniBossPrefabForEncounter = null;
        bossHasSpawned = false;
        ...
        ResumePressureEventsAfterMiniBoss();
    ...
    private void FindIntroBannerIfNeeded()
    {
        if (miniBossIntroBannerUI != null)
        {
            return;
        ...
        miniBossIntroBannerUI = FindObjectOfType<MiniBossIntroBannerUI>();
    }

    private void FindPressureEventDirectorIfNeeded()
    {
        if (pressureEventDirector != null)
        {
            return;
        ...
        pressureEventDirector = FindObjectOfType<EnemyPressureEventDirector>();
    }
    ...
    private void OnDisable()
    {
        ...
        if (miniBossIntroBannerUI != null)
        {
            miniBossIntroBannerUI.HideInstant();
        }
